...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    @annotation.tailrec
    def go(n: Int, currentNumber: Int, previousElement: Int, currentElement: Int): Int = {
      if (currentNumber == n) currentElement
      else go(n, currentNumber + 1, currentElement, previousElement + currentElement)
    }
